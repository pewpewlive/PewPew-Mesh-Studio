...
#pragma warning restore CS8974
            Log.Information("(Interpreter @ RunFile) <{Thread}> Plugin API initialized", Thread.CurrentThread.Name);
            lua.DoFile(LuaPath);
        }
    }
    public static void Run(string path)
    {
        try 
        {
            ...
            Log.Information("(Interpreter @ Run) <{Thread}> Spawning a new plugin thread...", Thread.CurrentThread.Name);
        ...
        {
            Log.Error(Ex, "(Interpreter @ Run) <Thread> Error encountered", Thread.CurrentThread.Name);
